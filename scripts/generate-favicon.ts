...
async function main() {
  ...
  const faviconDistPath = path.join(distDir, "favicon.png");
  ...
  const appleTouchIconDistPath = path.join(distDir, "apple-touch-icon.png");

  // Legacy .ico format
  const icoPath = path.join(__dirname, "../public/favicon.ico");
  const icoDistPath = path.join(distDir, "favicon.ico");
  ...
  try {
    ...
    if (fs.existsSync(distDir)) {
      fs.copyFileSync(faviconPath, faviconDistPath);
    }
    ...
    if (fs.existsSync(distDir)) {
      fs.writeFileSync(icoDistPath, icoBuffer);
    }
    ...
    await sharp(inputPath)
      .resize(180, 180, {
        fit: "contain",
        background: { r: 17, g: 20, b: 24, alpha: 1 },
      })
      .toFile(appleTouchIconPath);
    if (fs.existsSync(distDir)) {
      fs.copyFileSync(appleTouchIconPath, appleTouchIconDistPath);
    }
    console.log(
      `✅ Created apple-touch-icon.png (180x180) at ${appleTouchIconPath}`,
    );
  } catch (error) {
